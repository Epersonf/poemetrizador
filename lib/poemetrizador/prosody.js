...
export class Prosody {
  ...
  static rhymeKey(line){
    ...
    const last=ws[ws.length-1], syl=Syllabifier.syllabifyWord(last); if(!syl.length) return "";
    const back=Prosody.stressBackOffset(last), idx=syl.length-1-back;
    return syl.slice(idx).join("").replace(/[^\wáâãàéêôúõç]/g,"").replace(/^h/,"");
  }
  ...
  static peonio(stresses){
    return Prosody.patternLine("peonio (de segunda)", [2,6,10], "∪–∪∪", stresses);
  }

  static jonicoMaior(stresses){
    return Prosody.patternLine("jónico maior", [1,2,5,6,9,10], "––∪∪", stresses);
  }
}
