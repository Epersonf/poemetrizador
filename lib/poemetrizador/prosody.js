...
export class Prosody {
  static stressBackOffset(word) {
    const normWord = Utils.norm(word);
    // Tratamento especial para monosílabos átonos ou fracos em poesia (alinha a "um" e "eu")
    // Expandido para incluir preposições/conjunções átonas comuns (não recebem acento rítmico em métrica poética)
    const atonos = ["um", "uns", "eu", "com", "por", "em", "sem", "ou", "o", "a", "os", "as"];
    if (atonos.includes(normWord)) return 1; // Átonas não recebem acento rítmico
    // Pronomes relativos como "quem" são tônicos
    if (["quem"].includes(normWord)) return 0;
  ...
  static rhymeDisplay(line) {
    // Nova função para exibição da rima com acentos e cedilha preservados (alinha a "aço")
    const words = line.trim().split(/\s+/);
    // Melhorado para remover qualquer pontuação trailing (inclui em dash —, en dash –, etc., fixando bug em linhas com "—")
    // Remove pontuação (incluindo em dash, traços, etc.) e espaços à direita
    const lastWordRaw = words[words.length - 1].replace(/[\s—–.,;:!?]*$/i, "");
    const lastWord = lastWordRaw || words[words.length - 1]; // Fallback para evitar string vazia
    const normLast = Utils.norm(lastWord);
    const syl = Syllabifier.syllabifyWord(normLast);
    if (!syl.length) return "";
    const back = Prosody.stressBackOffset(normLast);
    const idx = syl.length - 1 - back;
    let part = syl.slice(Math.max(0, idx)).join("");
    const cleaned = part.replace(/^[^aeiouáâãàéêíóôúõ]+/i, "");
    const removedLen = part.length - cleaned.length;
    const originalStart = lastWord.length - part.length + removedLen;
    return lastWord.slice(originalStart);
  }
