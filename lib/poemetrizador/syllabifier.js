...
export class Syllabifier {
  ...
  static isHiato(word, i) {
    ...
    // 2ª vogal acentuada → hiato (pa-ís, sa-ú-de)
    if (/[áâãéêíóôú]/.test(b)) return true;

    const par = a + b;

    // REGRA ESPECIAL "iu":
    // - Se antes de "i" houver vogal → hiato (sa-iu, ca-iu, constru-iu)
    // - Caso contrário → ditongo (se-guiu, ri-u/viu/riu → 1 sílaba)
    if (par === "iu") {
      const prev = word[i - 1];
      let isPrevVowel = /[aeiouáâãàéêíóôúõ]/i.test(prev);
      if (Utils.REV_DIGRAPH_MAP.has(prev)) {
        const dig = Utils.REV_DIGRAPH_MAP.get(prev);
        isPrevVowel = /[aeiouáâãàéêíóôúõ]$/i.test(dig);
      }
      if (isPrevVowel) return true;
      return false;
    }

    // ditongos comuns (não separar)
    const DIT = new Set(["ai", "ei", "oi", "au", "eu", "ou", "ui", "ão", "õe", "ãi"]);

    if (DIT.has(par)) return false;

    // hiatos frequentes
    const HI = new Set(["ua", "uo", "eo", "oa", "oo", "ee", "ui"]);
    if (HI.has(par)) {
      // exceção QU/GU + a/o
